  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6">
        ...
        {showProactiveSuggestions && proactiveSuggestions.length > 0 && (
          <div className="mb-4 space-y-2">
            ...
            {proactiveSuggestions.map((suggestion) => (
              <div
                key={suggestion.id}
                className="flex items-center justify-between p-3 bg-purple-500/10 border border-purple-400/30 rounded-lg hover:bg-purple-500/20 transition-colors cursor-pointer group"
                onClick={() => handleProactiveSuggestionClick(suggestion)}
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-purple-200 text-sm">{suggestion.title}</span>
                  ...
                  <p className="text-xs text-purple-300/80">{suggestion.description}</p>
                </div>
                
                <div className="flex items-center gap-2">
                  <span className="text-xs text-purple-400">{suggestion.confidence}%</span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      dismissProactiveSuggestion(suggestion.id);
                    }}
                    className="p-1 rounded text-purple-400 hover:text-purple-200 opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                    ×
                  </button>
                </div>
        ...
        {/* Chat Messages */}
        <div className="h-64 overflow-y-auto space-y-3 mb-4 bg-slate-900/30 rounded-xl p-4 border border-slate-700/30">
          {messages.length === 0 && (
            <div className="text-center py-8">
              <Bot className="h-12 w-12 text-blue-400 mx-auto mb-4" />
              <h4 className="text-white font-semibold mb-2">Enhanced AI Console Ready</h4>
          ...
          {messages.map((message) => (
            ...
            >
              ...
              <div className={`max-w-sm ${message.type === 'user' ? 'order-2' : ''}`}>
                ...
                >
                  ...
                  {message.audioUrl && (
                    <button
                      ...
                      className="ml-2 p-1 rounded bg-green-500/20 text-green-400 hover:bg-green-500/30 transition-colors"
                    >
                ...
                {message.thinking && (
                  <div className="mt-2 p-2 bg-slate-600/30 rounded border-l-2 border-blue-500 text-xs text-gray-400">
                ...
                {message.toolsUsed && message.toolsUsed.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {message.toolsUsed.map((tool, index) => (
                      <span key={index} className="text-xs bg-orange-500/20 text-orange-300 px-2 py-1 rounded-full">
                        🔧 {tool}
                      </span>
                    ))}
                  </div>
          ...
          {isProcessing && (
            <div className="flex items-center gap-3">
              ...
              <div className="bg-slate-700/50 px-4 py-3 rounded-xl border border-slate-600/30">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-300">Enhanced AI processing with contextual understanding...</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mb-4 space-y-1">
            ...
            {commandSuggestions.map((suggestion, index) => (
              <button
                ...
                className="w-full text-left p-2 text-sm bg-slate-600/30 hover:bg-slate-600/50 border border-slate-500/30 hover:border-blue-500/30 rounded-lg transition-colors text-gray-300 hover:text-white"
              >
                {suggestion}
              </button>
            ))}
